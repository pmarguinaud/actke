...
INTEGER :: NPROMA
CHARACTER*64 :: CLCASE1, CLCASE2

CALL INITOPTIONS ()
CALL GETOPTION ("--case-1", CLCASE1, MND = .TRUE.)
CALL GETOPTION ("--case-2", CLCASE2, MND = .TRUE.)
NPROMA = 32
CALL GETOPTION ("--nproma", NPROMA)
CALL CHECKOPTIONS ()

CALL XRD_MKDIR (TRIM (CLCASE2))
...
OPEN (ILUNC, FILE=TRIM (CLCASE1)//"/ACTKE.COUNT", FORM="FORMATTED")
READ (ILUNC, *) NGPBLKS
CLOSE (ILUNC)

ALLOCATE (IFDIA (NGPBLKS))

CALL OPEN_LOAD (TRIM (CLCASE1)//"/ACTKE")
DO IBLOCK = 1, NGPBLKS
...
OPEN (ILUNC, FILE=TRIM (CLCASE2)//"/ACTKE.COUNT", FORM="FORMATTED")
WRITE (ILUNC, *) NGPBLKS
CLOSE (ILUNC)
...
CALL OPEN_SAVE (TRIM (CLCASE2)//"/ACTKE")
DO IBLOCK = 1, NGPBLKS
