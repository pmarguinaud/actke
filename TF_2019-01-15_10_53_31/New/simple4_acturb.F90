...
!- - - - - - - - - - - - - - - -
     ! ect moyenne de la cla (sans surf)
     ! qc_cloud moyen de la cla (sans surf)
    ! indice de couche melangee
      ! niveau de la base de l'inversion
...
INTEGER(KIND=JPIM) :: INQ1, JLEV, JLON  

LOGICAL :: LLIMQ1

REAL(KIND=JPRB) ::  Z2B, Z3B, Z3BCF, ZAA, ZCIS, ZCK, ZCTO, &
 & ZDD, ZDELTQF1, ZDELTQF2, ZDELTQH, &
 & ZDI, ZDIFFC, ZDIFFH, ZDLEWF1, ZDLEWF2, &
 & ZDPHI, ZDPHI0, ZDQW, ZDS, &
 & ZDTETA, ZDTL, ZECTH, ZEPDELT, ZEPNEBS, ZEPS1, ZEPSQ, ZEW1, ZEW2, ZGLMT2, ZGLMU2, ZGLT, ZGLTZ, ZGLU, ZGLUZ, &
 & ZGZ, ZH1, ZH2, ZIGMAS, ZIGMAS2, &
 & ZINC, ZIS, ZLMECT, ZLOI, ZLOS, &
 & ZLSCPEF1, ZLSCPEF2, ZLSCPEH, ZPREF, ZPRODC, ZPRODH, &
 & ZQ11, ZQC, ZQCF1, ZQCF2, ZQSATF1, ZQSATF2, ZQSLTLF1, ZQSLTLF2, ZQSLTLH, &
 & ZQWF1, ZQWF2, ZRH, ZRHOH, &
 & ZRLTLU, ZROSDPHI, ZRTI, ZSRC, ZSTA, ZTF1, ZTF2, ZTH, ZTHETAOT, &
 & ZTLF1, ZTLF2, ZU, ZWDIFF, ZWQW, ZWTL, ZZKTH, ZZRT, ZZT, ZL3F2, ZEPSIG
REAL(KIND=JPRB) ::  ZQV
...
INTEGER (KIND=JPIM) :: IPTRST_ZTHETALF

INTEGER (KIND=JPIM) :: IPTRST_ZTHETA

INTEGER (KIND=JPIM) :: IPTRST_ZGKUH

INTEGER (KIND=JPIM) :: IPTRST_ZGDZF

INTEGER (KIND=JPIM) :: IPTRST_ZLOCPEXF

INTEGER (KIND=JPIM) :: IPTRST_ZGKTH

INTEGER (KIND=JPIM) :: IPTRST_ZRTV

INTEGER (KIND=JPIM) :: IPTRST_ZRS

IPTRST = KPTRST

IPTRST_ZTHETALF = IPTRST
...
DO JLEV=KTDIAN,KLEV
  DO_JLON
    ...
    PSTACK (JLON,IPTRST_ZRTV+JLEV-1) = PR(JLON,JLEV)*PT(JLON,JLEV)
  ENDDO_JLON
ENDDO   ! JLEV
...
DO JLEV=KTDIAN,KLEV
  DO_JLON
    ...
    ZTHETAOT            = PSTACK(JLON,IPTRST_ZTHETA+JLEV-1)/ZZT
    PSTACK(JLON,IPTRST_ZLOCPEXF+JLEV-1) = PLSCPE(JLON,JLEV)*ZTHETAOT
    PSTACK(JLON,IPTRST_ZTHETALF+JLEV-1) = PSTACK(JLON,IPTRST_ZTHETA+JLEV-1)-ZQC*PSTACK(JLON,IPTRST_ZLOCPEXF+JLEV-1)
  ENDDO_JLON
ENDDO
!!
! - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
!     ------------------------------------------------------------------
DO_JLON
  PSTACK(JLON,IPTRST_ZGKUH+0-0:IPTRST_ZGKUH+KLEV-0)=1.E-14_JPRB
ENDDO_JLON
DO_JLON
  PSTACK(JLON,IPTRST_ZGKTH+0-0:IPTRST_ZGKTH+KLEV-0)=1.E-14_JPRB
ENDDO_JLON
DO JLEV=KTDIAN,KLEV-1
  DO_JLON

    ZDPHI    =PSTACK(JLON,IPTRST_ZGDZF+JLEV+1-1)
    ZZRT     =0.5_JPRB*(PSTACK(JLON,IPTRST_ZRTV+JLEV-1)+PSTACK(JLON,IPTRST_ZRTV+JLEV+1-1))
    ZROSDPHI =PAPRS(JLON,JLEV)/(ZDPHI*ZZRT)

    PSTACK(JLON,IPTRST_ZGKUH+JLEV-0)=AKN*PLMECT(JLON,JLEV)*SQRT(PECT(JLON,JLEV))
    PSTACK(JLON,IPTRST_ZGKTH+JLEV-0)=PSTACK(JLON,IPTRST_ZGKUH+JLEV-0)*PPHI3(JLON,JLEV)*ALPHAT

    PKTROV(JLON,JLEV)=PSTACK(JLON,IPTRST_ZGKTH+JLEV-0)*ZROSDPHI
    PKUROV(JLON,JLEV)=PSTACK(JLON,IPTRST_ZGKUH+JLEV-0)*ZROSDPHI
...
DO_JLON
  ...
  PGKCLS(JLON) = 0._JPRB
  PSTACK (JLON,IPTRST_ZGKTH+KLEV-0)= 0._JPRB
  PSTACK (JLON,IPTRST_ZGKUH+KLEV-0)= 0._JPRB
ENDDO_JLON
...
DO JLEV=KTDIAN,KLEV-1
  DO_JLON
    ...
    ZCIS  =MAX(ZEPS1,(PU(JLON,JLEV)-PU(JLON,JLEV+1))**2 &
     & +(PV(JLON,JLEV)-PV(JLON,JLEV+1))**2)  
    ZDTETA=PSTACK(JLON,IPTRST_ZRTV+JLEV-1)-PSTACK(JLON,IPTRST_ZRTV+JLEV+1-1)+RKAPPA*ZDPHI
    ZZRT  =2.0_JPRB/(PSTACK(JLON,IPTRST_ZRTV+JLEV-1)+PSTACK(JLON,IPTRST_ZRTV+JLEV+1-1))
...
!   En fait il faut 
    ZDTL   = (PSTACK(JLON,IPTRST_ZTHETALF+JLEV-1)-PSTACK(JLON,IPTRST_ZTHETALF+JLEV+1-1))*ZTH / &
            & (PSTACK(JLON,IPTRST_ZTHETALF+JLEV-1)+PSTACK(JLON,IPTRST_ZTHETALF+JLEV+1-1)) * 2._JPRB
