...
REAL(KIND=JPRB), PARAMETER :: ZN1D(-22:11) = (/ &
...
REAL(KIND=JPRB), PARAMETER :: ZRC1D(-22:11) = (/ &
...
REAL(KIND=JPRB), PARAMETER :: ZSRC1D(-22:11) = (/ &
...
!         - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    ...
    ZEW1     = EXP ( ( RALPW+ZH1*RALPD ) - ( RBETW+ZH1*RBETD ) / ZTLF1  - ( &
    & RGAMW+ZH1*RGAMD ) * LOG(ZTLF1) )/PAPRSF(JLON,JLEV)
    ZQSATF1  = ZEW1 / ( 1.0_JPRB+RETV*MAX(0.0_JPRB, (1.0_JPRB-ZEW1)) )
    ZDLEWF1  = ( ( RBETW+ZH1*RBETD ) - ( RGAMW+ZH1*RGAMD ) * ZTLF1 ) / ( ZTLF1*&
    & ZTLF1 )
    ZQSLTLF1 = ( ZQSATF1  * (1.0_JPRB-ZQSATF1)*ZDLEWF1 ) / (1.0_JPRB-ZEW1)
    ZDELTQF1 = ZQWF1-ZQSATF1
    ZDELTQF1 = SIGN(MAX(ABS(ZDELTQF1),ZEPDELT),ZDELTQF1)

    ZH2      = MAX(0.0_JPRB,SIGN(1.0_JPRB,RTT-ZTLF2))
    ZEW2     = EXP ( ( RALPW+ZH2*RALPD ) - ( RBETW+ZH2*RBETD ) / ZTLF2  - ( &
    & RGAMW+ZH2*RGAMD ) * LOG(ZTLF2) )/PAPRSF(JLON,JLEV+1)
    ZQSATF2  = ZEW2 / ( 1.0_JPRB+RETV*MAX(0.0_JPRB, (1.0_JPRB-ZEW2)) )
    ZDLEWF2  = ( ( RBETW+ZH2*RBETD ) - ( RGAMW+ZH2*RGAMD ) * ZTLF2 ) / ( ZTLF2*&
    & ZTLF2 )
    ZQSLTLF2 = ( ZQSATF2  * (1.0_JPRB-ZQSATF2)*ZDLEWF2 ) / (1.0_JPRB-ZEW2)
