...
& VKARMN, ALD, ECTMIN, ALMAVE, ARSC1, KPSTSZ, KPTRST, PSTACK)
...
DO JLEV=KTDIAN,KLEV
     ...
     ZGZBOT = (1.0_JPRB-PNLAB(JLON,JLEV))*ZGZBOT &
    &             + PNLAB(JLON,JLEV)*MIN(ZGZBOT,PAPHIF(JLON,JLEV))
...
DO JLEV=KTDIAN,KLEV
     ...
     ZGZBOTCVP = (1.0_JPRB-PNLABCVP(JLON,JLEV))*ZGZBOTCVP &
    &             + PNLABCVP(JLON,JLEV)*MIN(ZGZBOTCVP,PAPHIF(JLON,JLEV))
...
ENDDO
  ...
  ZTHETAH(JLON,KLEV)   = PTS*(RATM/PAPRS(JLON,KLEV))**(RKAPPA)
...
!     --------------------------------------
  ...
  DO JJLEV=JLEV,KTDIAN+1,-1
        ...
        ZDLUP2=( -(ZUSX-0.5_JPRB) +SIGN(1.0_JPRB,ZX)*&
         ...
         & +2.0_JPRB*ZUSX*ABS(ZEN/ZINCR))) )*ZDLUP1  
        ZDLUP =ZTEST*ZDLUP1+(1.0_JPRB-ZTEST)*ZDLUP2
        ZGLMUP(JLON,JLEV)=ZGLMUP(JLON,JLEV)+ZDLUP*ZTEST0
        ZEN   =ZEN  -ZINCR*ZTEST0
  ...
  DO JJLEV=JLEV+1,KLEV-1
        ...
        ZDLDN2=( -(ZUSX-0.5_JPRB) +SIGN(1.0_JPRB,ZX)*&
         & SQRT(ABS((ZUSX-0.5_JPRB)*(ZUSX-0.5_JPRB)&
         & +2.0_JPRB*ZUSX*ABS(ZEN/ZINCR))) )*ZDLDN1  
        ZDLDN=ZTEST*ZDLDN1+(1.0_JPRB-ZTEST)*ZDLDN2
        ZGLMDN(JLON,JLEV)=ZGLMDN(JLON,JLEV)+ZDLDN*ZTEST0
        ZEN   =ZEN  -ZINCR*ZTEST0
  ...
  ! - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    ...
    ZGZLCVPUP = MIN(RG*ALMAVX,ZGZTOPCVP-PAPHI(JLON,JLEV))
    ZGZLCVPDN = MIN(RG*ALMAVX,PAPHI(JLON,JLEV)-ZGZBOTCVP)
