...
all: wrap_actke.x 
#all: wrap_actke.x main_simple2_actke.x

clean:
	\rm -f *.xml *.o *.mod *.x *.optrpt


FRTFLAGS = -convert big_endian -assume byterecl -align array64byte -traceback -fpic -qopenmp -qopenmp-threadprivate compat -fp-model source -qopt-report=5 -qopt-report-phase=vec -ftz -g -O2 -axCORE-AVX2 -finline-functions -finline-limit=500 -Winline -qopt-prefetch=4 -fast-transcendentals -no-fma

FC = ifort -DCPU $(FRTFLAGS)
...
wrap_actke.x: wrap_actke.o run_simple4_actke.o simple4_acbl89.o  simple4_acevolet.o  simple4_actke.o  simple4_acturb.o  simple4_fl2hl.o  simple4_hl2fl.o parkind1.o load_mod.o yomlog.o xrd_unix_env.o xrd_getoptions.o set.o
	$(FC) -o wrap_actke.x wrap_actke.o run_simple4_actke.o simple4_acbl89.o  simple4_acevolet.o  simple4_actke.o  simple4_acturb.o  simple4_fl2hl.o  simple4_hl2fl.o parkind1.o load_mod.o yomlog.o xrd_unix_env.o xrd_getoptions.o set.o
